...
-- Create custom BufDel command to delete the current buffer
vim.api.nvim_create_user_command('BufDel', function() vim.cmd 'bdelete' end, { desc = 'Delete the current buffer' })
vim.keymap.set('n', '<leader>b', ':BufDel<cr>', { desc = '[D]eletes the current buffer' })
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<C-f>', builtin.live_grep, { desc = 'Search with grep' })
  ...
  },
  {
    ...
    keys = {
      { '<leader>k', function() vim.diagnostic.jump { count = -1, float = true } end, desc = 'Go to prev diagnostic [k]' },
      { '<leader>j', function() vim.diagnostic.jump { count = 1, float = true } end, desc = 'Go to next diagnostic [j]' },
    },
  },
